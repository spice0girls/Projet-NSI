...
scroll = 0
continuous_scroll = 0
#scroll qui peut que monté pour ne pas avoir de respawn des ennemis après avoir reculé
scroll_limitd = 150
scroll_limitg = 80
floor = 190
last_floor = floor
...
def perso_deplacement(x, y, jump, scroll, last_scroll, last_floor, monte, descente, continuous_scroll):
    last_scroll = scroll
    ...
    if jump == True:
        if last_floor - taille_saut - taille_perso_y <= y and descente == False:
            ...
            if y + taille_perso_y + pvitesse_mouv_bas >= floor:
                ...
                jump = False
                return x, y, jump, scroll, last_scroll, last_floor, monte, descente, continuous_scroll
...
def ennemi_movement_y(ennemi):
    for plateforme in plateforme_liste:
        if ennemi[1] + taille_ennemi_y <= plateforme[1] and ennemi[0] + taille_ennemi_x > plateforme[0] and ennemi[0] < plateforme[0] + taille_plateforme:
...
def update():
    ...
    global perso_x, perso_y, jump, scroll, last_scroll, plateforme_liste, floor, pvitesse_mouv_dg, last_floor, monte, descente, playing, ennemi_liste,ennemi_floor_base, continuous_scroll
    ...
    if scroll >= 120:
        playing = 2
    if playing == 1:
        perso_x, perso_y, jump, scroll, last_scroll, last_floor, monte, descente, continuous_scroll= perso_deplacement(perso_x, perso_y, jump, scroll, last_scroll, last_floor, monte, descente,continuous_scroll)
        ...
        if continuous_scroll == 14 and last_scroll!=scroll:
            ennemi_creation(ennemi_liste,256,floor-taille_ennemi_y,floor)
        if continuous_scroll==35 and last_scroll!=scroll:
            ennemi_creation(ennemi_liste,256,90,100+taille_ennemi_y)
        if last_scroll != scroll:
