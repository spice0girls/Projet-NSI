...
def perso_deplacement():
    ...
    global perso_x, perso_y, jump, scroll, last_scroll, last_floor, monte, descente, \
        continuous_scroll, vie, ennemi_liste, last_perso_x
    last_scroll = scroll
...
def update():
    ...
    global scroll, last_scroll, plateforme_liste, floor, playing, continuous_scroll, taille_ennemi_y, vie, \
        niveau, compteur, ennemi_vitesse_mouv, projectile_vitesse, fleur_liste, arbre_liste, buisson_liste
...
def draw():
    ...
    global background, floor, compteur_animation_1, compteur_animation_2, touche
    pyxel.cls(0)
    ...
    if playing == 2:  # écran qui s'affiche quand on gagne
        pyxel.text(108, 107, "with", 9)
        pyxel.text(138, 107, str(points), 9)
        pyxel.blt(128, 105, 0, 85, 53, taille_fleur_x, taille_fleur_y, 0)
        pyxel.blt(90, 60, 0, 16, 96, 80, 32, 0)
