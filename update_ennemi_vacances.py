...
plateforme_liste=[]
taille_plateforme = 8
plvitesse_mouv = pvitesse_mouv_dg
...
def ennemi_creation(x, y, ennemi_floor,projectile,reste_sur_plateforme):
    """Création d'ennemi dependant un niveau"""
    # dans ennemi_liste: x de l'ennemi, y de l'ennemi, le sol de l'ennemi, projectile ou non?, reste sur plateforme ou non?
    ennemi =[x, y, ennemi_floor]
    if projectile == False:
        ennemi.append(False)
    else:ennemi.append(True)

    if reste_sur_plateforme == True:
        ennemi.append(True)
    else: ennemi.append(False)
...
def ennemi_deplacement():
    ...
    for ennemi in ennemi_liste:
        ...
        else: check_mouv_dg_ennemi(ennemi)
...
def check_mouv_dg_ennemi(ennemi):
    ...
    if ennemi[4]:
        for plateforme in plateforme_liste:
            if not ennemi[5]:
                if ennemi[1] + taille_ennemi_y == plateforme[1] and ennemi[0] < plateforme[0] < ennemi[0] + taille_ennemi_x:
                    for plateforme2 in plateforme_liste:
                        if ennemi[1] + taille_ennemi_y == plateforme2[1] and ennemi[0] < plateforme2[0] + taille_plateforme < ennemi[0] + taille_ennemi_x:
                            ennemi[5] = False
                    ...
                    return
            elif ennemi[1] + taille_ennemi_y == plateforme[1] and ennemi[0] < plateforme[0] + taille_plateforme < ennemi[0] + taille_ennemi_x:
                for plateforme2 in plateforme_liste:
                    if ennemi[1] + taille_ennemi_y == plateforme2[1] and ennemi[0] < plateforme2[0] < ennemi[0] + taille_ennemi_x:
                        ennemi[5]= True
...
def reset():
    ...
    global perso_x, perso_y, jump, monte, descente, scroll, continuous_scroll,floor,last_floor,last_scroll,vie,ennemi_liste,ennemi_floor_base, ennemi_vitesse_mouv, projectile_vitesse, projectile_liste
    perso_x = 128
    perso_y = 182
    jump = False
    monte = False
...
def update():
    ...
    if playing == 1:
        if niveau == 1:
            if compteur==0:
                ...
                plateforme_liste = [[ecran_bord, hauteur_1, 1], [ecran_bord, hauteur_1, 3], [ecran_bord, hauteur_1, 5],
                                    [ecran_bord, hauteur_1, 16], [ecran_bord, hauteur_1, 18],[ecran_bord, hauteur_1, 20],
                                    [ecran_bord, hauteur_2, 25],[ecran_bord, hauteur_2, 27], [ecran_bord, hauteur_2, 29],
                                    [ecran_bord, hauteur_1, 35], [ecran_bord, hauteur_1, 37],[ecran_bord, hauteur_1, 39],
                                    [ecran_bord, hauteur_1, 41],[ecran_bord, hauteur_1, 43],[ecran_bord, hauteur_1, 45],
                                    [ecran_bord, hauteur_2, 60],[ecran_bord, hauteur_2, 62],[ecran_bord, hauteur_2, 64]]
            ...
            if (continuous_scroll == 14 or continuous_scroll == 50 )and last_scroll != scroll:
                ennemi_creation(ecran_bord, floor - taille_ennemi_y, floor,False,False) #création d'ennemi au sol normal sans projectile
            if continuous_scroll==45 and last_scroll!=scroll:
                ennemi_creation(ecran_bord,hauteur_1-taille_ennemi_y, hauteur_1,False,True) #création d'ennemi à l'hauteur 1 sans projectile
            if continuous_scroll == 62 and last_scroll != scroll:
                ennemi_creation(ecran_bord, hauteur_2 - taille_ennemi_y, hauteur_2,False,False)  # création d'ennemi à l'hauteur 2 sans projectile
        ...
        if niveau == 2:
            ...
            if (continuous_scroll == 14 or continuous_scroll == 50 )and last_scroll != scroll:
                ennemi_creation(ecran_bord, floor - taille_ennemi_y, floor,False,False)
            if continuous_scroll==39 and last_scroll!=scroll:
                ennemi_creation(ecran_bord,hauteur_1-taille_ennemi_y, hauteur_1,False,True)
            if continuous_scroll==29 and last_scroll!=scroll:
                ennemi_creation(ecran_bord,hauteur_2-taille_ennemi_y, hauteur_2,False,False)
            if scroll >= 120:
                playing = 2
        if niveau == 3:
            if compteur==0:
            ...
            if (continuous_scroll == 14 or continuous_scroll == 50) and last_scroll != scroll:
                ennemi_creation( ecran_bord, floor - taille_ennemi_y, floor,True,False)
            if continuous_scroll == 39 and last_scroll != scroll:
                ennemi_creation( ecran_bord, hauteur_1 - taille_ennemi_y, hauteur_1,True,False)
            if scroll >= 120:
                playing = 2
