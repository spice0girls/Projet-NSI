...
def fleur_deplacement():
    """Déplacement des plateformes"""
    if last_scroll > scroll:
        for fleur in fleur_liste:
            if fleur[2] < scroll:
                fleur[0] += plvitesse_mouv
        return
    if last_scroll < scroll:
        for fleur in fleur_liste:
            if fleur[2] < scroll:
...
def update():
    ...
    global scroll, last_scroll, plateforme_liste, floor, playing, continuous_scroll, taille_ennemi_y, vie, \
        niveau, compteur, ennemi_vitesse_mouv, projectile_vitesse, fleur_liste
    ...
    if pyxel.btnp(pyxel.KEY_F):
        print("here", perso_x, perso_y, scroll)
    ...
    if playing == 1:
        ...
        if niveau == 2:
            ...
            if (
                    continuous_scroll == 14 or continuous_scroll == 50 or continuous_scroll == 90 or continuous_scroll == 200 or continuous_scroll == 230 or continuous_scroll == 249 or continuous_scroll == 290) and last_scroll != scroll:
                ennemi_creation(floor - taille_ennemi_y, floor, False, False)
            if (
                    continuous_scroll == 41 or continuous_scroll == 180 or continuous_scroll == 302) and last_scroll != scroll:
...
def draw():
    ...
    if playing == 1:  # écran quand on joue
        if compteur != 0:
            if (pyxel.frame_count % 10) == 0:
                ...
                if background >= 256:
                    background = 0
            pyxel.bltm(0, 0, 0, background, 0, 256, ennemi_floor_base)
            pyxel.bltm(0, ennemi_floor_base, 0, 0, ennemi_floor_base, 256, 66)
    ...
    if playing == 2:  # écran qui s'affiche quand on gagne
        pyxel.text(128, 57, str(points), 9)
        pyxel.blt(118, 55, 0, 85, 53, taille_fleur_x, taille_fleur_y, 0)
