...
def reset():
    ...
    global perso_x, perso_y, jump, monte, descente, scroll, continuous_scroll, floor, last_floor, \
        last_scroll, vie, ennemi_liste, ennemi_vitesse_mouv, projectile_vitesse, projectile_liste, background
    perso_x = 128
    perso_y = 182
    jump = False
    monte = False
...
def update():
    ...
    if playing == 1:
        if niveau == 1:
            ...
            if (
                    continuous_scroll == 20 or continuous_scroll == 60 or continuous_scroll == 240) and last_scroll != scroll:
    ...
    if playing == 1:  # écran quand on joue
        if compteur != 0:
            if (pyxel.frame_count % 10) == 0:
                ...
                if background >= 256:
                    background = 0
            pyxel.bltm(0, 0, 0, background, 0, 256, 190)
            pyxel.bltm(0, 190, 0, 0, ennemi_floor_base, 256, 66)
